...
import by.alexeysavchic.exception.CannotRewriteXmlFileException;
import by.alexeysavchic.exception.ChangingFileDuringExecutingProgramException;
    ...
    public synchronized List<GetDataFromXmlDTO> getActualWarehouseInfo(LocalDateTime time) {
        if (isValid()) {
            ...
            try (StreamContext streamContext = new StreamContext(xmlPath, intermediateXml, inputFactory, outputFactory)) {
                ...
                throw new ChangingFileDuringExecutingProgramException(xmlPath.toString());
            } catch (IOException e) {
    ...
    public synchronized void setWarehouseInfo(List<UpdateWarehouseDTO> updateList) {
        if (isValid()) {
            ...
            try (StreamContext streamContext = new StreamContext(xmlPath, intermediateXml, inputFactory, outputFactory)) {
                ...
                if (isWriting) {
                    throw new XmlWritingException(intermediateXml.toString());
                } else {
                    throw new ChangingFileDuringExecutingProgramException(xmlPath.toString());
                }
            }
            ...
            try {
                ...
                throw new CannotRewriteXmlFileException(xmlPath.toString());
            }
        } else {
            throw new InvalidXmlFileException();
        }
